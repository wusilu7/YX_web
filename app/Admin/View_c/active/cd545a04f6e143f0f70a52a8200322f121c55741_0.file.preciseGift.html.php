<?php
/* Smarty version 3.1.30, created on 2024-08-15 10:00:28
  from "D:\pro\WebSiteYiXing\app\Admin\View\active\preciseGift.html" */

/* @var Smarty_Internal_Template $_smarty_tpl */
if ($_smarty_tpl->_decodeProperties($_smarty_tpl, array (
  'version' => '3.1.30',
  'unifunc' => 'content_66bd613c1faf08_00401773',
  'has_nocache_code' => false,
  'file_dependency' => 
  array (
    'cd545a04f6e143f0f70a52a8200322f121c55741' => 
    array (
      0 => 'D:\\pro\\WebSiteYiXing\\app\\Admin\\View\\active\\preciseGift.html',
      1 => 1685435975,
      2 => 'file',
    ),
  ),
...
function content_66bd613c1faf08_00401773 (Smarty_Internal_Template $_smarty_tpl) {
...
<div class="jin-content-title"><span>精准礼包</span></div>
<div class="alert alert-info">
    ...
    <button id="jin_sync0" class="btn btn-info">同步欧美</button>
    <button id="jin_sync2" class="btn btn-info">同步海外</button>
...
<?php echo '<script'; ?>
 type="text/javascript">
    ...
    var Reward = function (json) {
        return '<div style="width: 200px; margin: 0 auto;">'+json['Reward']+'</div>';
    };
    var ShowReward1 = function (json) {
        return '<div style="width: 200px; margin: 0 auto;">'+json['ShowReward1']+'</div>';
    };
    var RewardRandPool = function (json) {
        return '<div style="width: 200px; margin: 0 auto;">'+json['RewardRandPool']+'</div>';
    };
    var ContinueTime = function (json) {
        return Math.floor(json['ContinueTime']/60)+'小时'+json['ContinueTime']%60;
    };
    var Send_si = function (json) {
        return '<a class="btn btn-info" data-info="' + json['send_si_res'] + '"><span data-data-si="'+json['send_si']+'" class="glyphicon glyphicon-info-sign">查看</span></a>';
    };
    var SKU = function (json) {
        return '<span>安卓:'+json['SKUAndroid']+'</span><br><span>IOS:'+json['SKUIOS']+'</span>'
    };
    var arr = [id_check,'Sort','GiftName','PayType','IsOpen','OpenTime','EndTime',ContinueTime,'LimitCount','Cost','Price','Condition','UpdateTime','ProductID',SKU,Reward,Send_si,'send_time',btn];
    // 普通查询
    ...
    $("#jin_sync0").on('click', function () {  // 点击后本地数据库
        sync(0);
    });
    $("#jin_sync1").on('click', function () {  // 点击后本地数据库
        sync(1);
    });
    $("#jin_sync2").on('click', function () {  // 点击后本地数据库
        sync(2);
    });
    ...
    function sync111(s_type) {
        layer.alert('确认同步吗？', {icon: 0, btn: ['确定', '取消'], shadeClose: true}, function () {
            $.ajax({
                type: "POST",
                url: location.href + "&jinIf=921",
                data: {
    ...
    function common() {
        if ($.cookie('cookie_sign2')) {
            $("#sign option[value='"+$.cookie('cookie_sign2')+"']").attr("selected","selected");
        }
        data.gi=$("#group").val();
        data.sign = $("#sign").val();
        $.cookie('cookie_sign2', $("#sign").val(), {expires: 1});
