<?php
/* Smarty version 3.1.30, created on 2024-08-16 10:31:30
  from "D:\phpStudy\PHPTutorial\WWW\WebSiteYiXing\app\Admin\View\operation\ActiveListClient.html" */

/* @var Smarty_Internal_Template $_smarty_tpl */
if ($_smarty_tpl->_decodeProperties($_smarty_tpl, array (
  'version' => '3.1.30',
  'unifunc' => 'content_66beba02d68b13_39726753',
  'has_nocache_code' => false,
  'file_dependency' => 
  array (
    '7ba7c24039460740e13d4e489bc1965515a6805d' => 
    array (
      0 => 'D:\\phpStudy\\PHPTutorial\\WWW\\WebSiteYiXing\\app\\Admin\\View\\operation\\ActiveListClient.html',
      1 => 1723704876,
      2 => 'file',
    ),
  ),
...
function content_66beba02d68b13_39726753 (Smarty_Internal_Template $_smarty_tpl) {
...
<div class="jin-content-title"><span>热更活动表Client</span></div>
        ...
        $.ajax({
            ...
            success: function (json) {
                ...
                for (var i=0;i<json.length;i++){
                    ...
                    client_tb_id = json[i]['client_tb_id'];
                    c+='<div class="input-group"><span class="input-group-addon">'+json[i]['filed_name']+'( <span style="color: red;">'+json[i]['client_col_id']+'</span>)</span>' +
                        '<input type="text" id="'+json[i]['filed_name']+'" data-data-tb_path="'+json[i]['tb_path']+
                        '" data-data-client_tb_id="'+json[i]['client_tb_id']+'" data-data-client_col_id="'+json[i]['client_col_id']+
                ...
                layer.open({
                    ...
                    '<div class="input-group"><span class="input-group-addon">表映射ID</span><input type="text" value="'+client_tb_id+'" disabled  class="form-control"></div>' +
                    '<div class="input-group"><span class="input-group-addon">表路径</span><input type="text"  value="'+$("#tb_path").val()+'" disabled  class="form-control"></div>' +
                    c+'</div>',
                    yes: function (index1) {
                        var arr_tb_body =[];
        ...
        $.ajax({
            ...
            success: function (json) {
                ...
                for (var i=0;i<json.length;i++){
                    ...
                    arr_filed.push(json[i]['server_col_idx']);
                    c+='<div class="input-group"><span class="input-group-addon">'+json[i]['server_col_idx']+'( <span style="color: red;">'+json[i]['client_col_idx']+'</span>)</span></span>' +
                        '<input type="text" '+is_disabled+'  id="'+json[i]['server_col_idx'].trim()+'" value="'+json[i]['client_value']+'" ' +
                        'data-data-client_row_idx="'+json[i]['client_row_idx']+'"  class="form-control filed-width">' +
                        '<input '+is_disabled+' style="margin-left: 10px; width: 20px; height: 20px;" '+nocnv+' type="checkbox" id="'+json[i]['server_col_idx']+'_nocnv" value="1">冒号转化' +
                        '</div>';
                }
                layer.open({
                    type: 1,
                    ...
                    yes: function (index1) {
                        ...
                        for (var i=0;i<arr_filed.length;i++){
                            var json_temporary={
                                ...
                                'client_value':$("#"+arr_filed[i]).val(),
                                'nocnv':$("#"+arr_filed[i]+"_nocnv").is(':checked') ? $("#"+arr_filed[i]+"_nocnv").val() : 0,
                                'client_row_idx':$("#"+arr_filed[i]).attr('data-data-client_row_idx')
                            };
                            arr_tb_body.push(json_temporary);
                        }
                        $.ajax({
    ...
    $("#delete_all").on('click',function(){
        ...
        layer.alert('确认操作吗？', {icon: 0, btn: ['确定', '取消'], shadeClose: true}, function () {
            $.ajax({
                ...
                data: {
                    id: IDs,
                    tb_path: $("#tb_path").find("option:selected").text(),
                    si:$("#s").val()
                },
                dataType: "json",
                beforeSend: function () {
