...
class RetentionDeviceTaskModel1 extends XoaModel
{
    // 设备留存率定时任务
    function ODMRetentionDevice($date='')
    {
        ...
        if (!empty($res_u)) {
            foreach ($res_u as $v) {
                txt_put_log('retention_device1', '更新数据失败', '记录时间：' . date('Y-m-d H:i:s') . ',渠道id、设备类型 ：' . $v);  //日志记录
            }
        }

        if (!empty($res_i)) {
            foreach ($res_i as $v) {
                txt_put_log('retention_device1', '插入数据失败', '记录时间：' . date('Y-m-d H:i:s') . ',渠道id、设备类型 ：' . $v);  //日志记录
            }
        }
    ...
    function check_rd_data($date, $type='i')
    {
        ...
        foreach ($groupArr as $g) {
            if ($type == 'i') {
                ...
                $sql = "select `gi` from retention_device1 where gi=? and `date` = ?";
    ...
    function insertNumUp($date, $gi, $devicetype)
    {
        ...
        unset($arrUp);
        $sql = "insert into retention_device1(`date`,gi,devicetype,numup) values(?,?,?,?)";
        $res = $this->go($sql, 'i', [$date, $gi, $devicetype, $numUp]);
        if ($res !== false) {
            txt_put_log('retention_device1', '插入数据成功', '记录时间：' . date('Y-m-d H:i:s') . ',新增服id：' . $gi.',新增平台id：' . $devicetype);  //日志记录
            unset($res);
        } else {
    ...
    function updateRetention($date, $n, $gi, $devicetype)
    {
        ...
        $rColumn = 'r' . $n;
        $sql = "update retention_device1 set $numInColumn=?, $rColumn=? where `date`=? and gi=? and `devicetype`=?";
        $res = $this->go($sql, 'u', [$numIn, $r, $dateN, $gi, $devicetype]);
        if ($res !== false) {
            txt_put_log('retention_device1', '更新成功', '记录时间：' . date('Y-m-d H:i:s') . ',修改内容：' . $numIn . '_' . $r.',更新服id：' .$gi. ',更新平台id：' . $devicetype. ',留存第几天：' . $n);  //日志记录
        } else {
            $errData[] = $gi . '_' . $devicetype;
        }
    ...
    function deviceDayUp($date, $gi, $devicetype)
    {
        ...
        $sql = "select code from loginLog  where gi=".$gi." AND pi in (8,11) and opt1_group=1 and time>='".$dateStart."' AND time<='".$dateEnd."'";
        if ($devicetype > 0) {
