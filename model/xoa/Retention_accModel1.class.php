...
class Retention_accModel1 extends XoaModel
{
    ...
    function insertSumData2($arr)
    {
        ...
        $sql = 'SELECT sum(numup) numup, sum(numin1) numin1, sum(numin2) numin2, sum(numin3) numin3, sum(numin4) numin4, sum(numin5) numin5, sum(numin6) numin6, sum(numin7) numin7, sum(numin14) numin14, sum(numin29) numin29,sum(numin44) numin44, sum(numin59) numin59 from retention_acc1';
        $sql .= ' where gi in ('.implode(',', POST('group')).')';

        $sql2 = 'SELECT date,numup, numin1, numin2, numin3, numin4, numin5, numin6, numin7, numin14, numin29, numin44, numin59 from retention_acc1';
    ...
    {
        $sql1 = "select * from retention_acc1 ";
        $sql2 = " where date< ?";
    ...
    function putNormalData2($arr)
    {
        ...
        $arrUp = $ratm->signupCount2(date('Y-m-d'), $this->group_id,  $this->platform_id);
        ...
        foreach ($day as $d) {
                foreach ($this->group_id as $k => $gi) {
                    $res = $ratm->computeRetention(date('Y-m-d'), $d, $gi , $this->platform_id);
    ...
    function putNormalData4($arr)
    {
        ...
        $ratm = new RetentionAccTaskModel1;
        $arrUp = $ratm->signupCount2(date('Y-m-d'), $groupsArr, $this->platform_id);
        $numUp = count($arrUp);//查询当日安装数
        $dailyarr = array(
            "date" => date('Y-m-d') . '<b><div>(实时更新)</div></b>',
            'numup' => $numUp,
