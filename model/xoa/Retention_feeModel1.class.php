...
class Retention_feeModel1 extends XoaModel
{
    ...
    function insertSumData2($arr)
    {
        ...
        $sql1 = 'SELECT sum(numup) numup, sum(numin1) numin1, sum(numin2) numin2, sum(numin3) numin3, sum(numin4) numin4, sum(numin5) numin5, sum(numin6) numin6, sum(numin7) numin7, sum(numin14) numin14, sum(numin29) numin29,sum(numin44) numin44, sum(numin59) numin59 from retention_fee1';
        $sql2 = 'SELECT date,numup, numin1, numin2, numin3, numin4, numin5, numin6, numin7, numin14, numin29,numin44, numin59 from retention_fee1';
    ...
    {
        $name = 'R_Fee_type' . '_' . date('Ymd_His');
        $excel = new Excel;
        $excel->setTitle($name);
    ...
    function putNormalData2($arr)
    {
        ...
        $arrUp = $rdtm->deviceDayUp(date('Y-m-d'), implode(',',$this->group_id), $this->platform_id);
    ...
    function putNormalData4($arr){
        ...
        //头上插入当天实时安装数 (999)
        $rdtm = new RetentionFeeTaskModel1;
        $arrUp = $rdtm->deviceDayUp(date('Y-m-d'), $groups, $this->platform_id);
        $numUp = count($arrUp);//查询当日安装数
        $dailyarr = array(
            "date" => date('Y-m-d'). '<b><div>(实时更新)</div></b>',
            'numup' => $numUp,
