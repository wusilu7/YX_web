...
class Retention_accModel extends XoaModel
{
    ...
    function insertSumData2($arr)
    {
        ...
        $sql = 'SELECT sum(numup) numup, sum(numin1) numin1, sum(numin2) numin2, sum(numin3) numin3, sum(numin4) numin4, sum(numin5) numin5, sum(numin6) numin6, sum(numin7) numin7, sum(numin8) numin8, sum(numin9) numin9, sum(numin14) numin14, sum(numin29) numin29,sum(numin44) numin44, sum(numin59) numin59 from retention_acc';
        $sql .= ' where si in ('.implode(',', POST('si')).')';

        $sql2 = 'SELECT date,numup, numin1, numin2, numin3, numin4, numin5, numin6, numin7, numin8, numin9, numin14, numin29, numin44, numin59 from retention_acc';
    ...
    {
        // $check = $this->check_retention_acc();
        $sql1 = "select * from retention_acc ";
        $sql2 = " where date< ?";
    ...
    {
        $name = 'R_acc_type' . '_' . date('Ymd_His');
        $excel = new Excel;
        $excel->setTitle($name);
        $excel->setCellTitle('a1', '日期');
        $excel->setCellTitle('b1', '注册帐号数');
        $excel->setCellTitle('c1', '1天后登陆数');
        $excel->setCellTitle('d1', '次日留存');
        $excel->setCellTitle('e1', '2天后登陆数');
        $excel->setCellTitle('f1', '2日留存率');
        $excel->setCellTitle('g1', '3天后登陆数');
        $excel->setCellTitle('h1', '3日留存率');
        $excel->setCellTitle('i1', '4天后登陆数');
        $excel->setCellTitle('j1', '4日留存率');
        $excel->setCellTitle('k1', '5天后登陆数');
        $excel->setCellTitle('l1', '5日留存率');
        $excel->setCellTitle('m1', '6天后登陆数');
        $excel->setCellTitle('n1', '6日留存率');
        $excel->setCellTitle('o1', '7天后登陆数');
        $excel->setCellTitle('p1', '7日留存率');
        $excel->setCellTitle('q1', '8天后登陆数');
        $excel->setCellTitle('r1', '8日留存率');
        $excel->setCellTitle('s1', '9天后登陆数');
        $excel->setCellTitle('t1', '9日留存率');
        $excel->setCellTitle('u1', '10天后登陆数');
        $excel->setCellTitle('v1', '10日留存率');
        $excel->setCellTitle('w1', '15天后登陆数');
        $excel->setCellTitle('x1', '15日留存率');
        $excel->setCellTitle('y1', '30天后登陆数');
    ...
    function putNormalData2($arr)
    {
        ...
        $arrUp = $ratm->signupCount2(date('Y-m-d'), $this->group_id, $this->server_id, $this->platform_id);
        ...
        foreach ($day as $d) {
                foreach ($this->server_id as $k => $si) {
                    $res = $ratm->computeRetention(date('Y-m-d'), $d ,$si, $this->group_id);
    ...
    function putNormalData4($arr)
    {
        ...
        $ratm = new RetentionAccTaskModel;
        $arrUp = $ratm->signupCount2(date('Y-m-d'), $this->group_id, $siArr, $this->platform_id);
        $numUp = count($arrUp);//查询当日安装数
        $dailyarr = array(
            "date" => date('Y-m-d') . '<b><div>(实时更新)</div></b>',
            'numup' => $numUp,
        ...
        foreach ($day as $d) {
                foreach ($siArr as $k => $si) {
                    $res = $ratm->computeRetention(date('Y-m-d'), $d,$si, $this->platform_id);
                    $numInColumn = 'numin' . $d;//字段名拼接
                    $rColumn = 'r' . $d;
                    foreach ($arr as $ak=>$av){
                        if($av['date']==date('Y-m-d', strtotime('-'.$d.' day'))){
    ...
    function check_retention_acc()
    {
        $sql = 'SELECT `date` from `retention_acc` order by `date` DESC';
        $check = $this->go($sql, 's');
        ...
        if ($check['date'] != $newday) {
            ...
            $days = $days / $oneday;
            $date = '';
            $ltm = new RetentionAccTaskModel;
            for ($i=1; $i <= $days; $i++) {
                $date = date('Y-m-d',strtotime($check['date'] . '+' . $i . ' day'));
                $ltm->ODMRetentionAcc($date);
            // var_dump($date);die;
            }
        }
    }
    ...
    function raWrong($si)
    {
        ...
        foreach ($si as $v) {
            $sql = 'SELECT `date`, gi, si, numup from `retention_acc` where devicetype = 0 and `date` = ? and si = ?';
            $param = [
                $date,
                $v['server_id']
            ];
        ...
        foreach ($res as $v) {
            if ($v['gi'] == '' && $v['si'] == '' && $v['date'] == '' && $v['numup'] == '') {
               $wrong[] = '基础数据 : 账号留存率 —> '.$v['group_name'].' —> '.$v['server_name']." ( {$date} 数据异常 )"; 
            }
        }
        if ($wrong == '') {
            $wrong = "基础数据 : 账号留存率 ( {$date} 数据正常)";
        }
