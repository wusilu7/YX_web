...
class Retention_accModel2 extends XoaModel
{
    ...
    function insertSumData2($arr)
    {
        ...
        $sql .=' sum(numchar_a30) numchar_a30, sum(numchar_b30) numchar_b30, sum(numchar_c30) numchar_c30, sum(numchar_a60) numchar_a60, sum(numchar_b60) numchar_b60, sum(numchar_c60) numchar_c60 from retention_acc2';
        $sql .= ' where gi in ('.implode(',', $gi).')';
    ...
    function getRetention($time_start = '', $time_end = '')
    {
        ...
        $sql1 = "select * from retention_acc2 where 1=1 ";
        $sql2 = " ";
    ...
    {
        $rdtm = new RetentionAccTaskModel2;
        $arrUp = $rdtm->signupCount2(date('Y-m-d'), implode(',',$this->group_id), $this->platform_id);
    ...
    function putNormalData4($arr)
    {
        ...
        //头上插入当天实时安装数 (999)
        $rdtm = new RetentionAccTaskModel2;
        $arrUp = $rdtm->signupCount2(date('Y-m-d'), $groups, $this->platform_id);
