...
class RetentionFeeTaskModel extends XoaModel
{
    // 设备留存率定时任务
    function ODMRetentionDevice($day_three='')
    {
        ...
        if (!empty($res_u)) {
            foreach ($res_u as $v) {
                txt_put_log('retention_fee', '更新数据失败', '记录时间：' . date('Y-m-d H:i:s') . ',渠道id、设备类型 ：' . $v);  //日志记录
            }
        }

        if (!empty($res_i)) {
            foreach ($res_i as $v) {
                txt_put_log('retention_fee', '插入数据失败', '记录时间：' . date('Y-m-d H:i:s') . ',渠道id、设备类型 ：' . $v);  //日志记录
            }
        }
        ...
        foreach ($serverArr as $k=>$si) {
            ...
            foreach ($si as $s){
                if ($type == 'i') {
                    ...
                    $sql = "select `si` from retention_fee where si=? and `date`=? AND gi=?";
    ...
    function insertNumUp2($date, $si, $gi)
    {
        ...
        unset($arrUp);
        $sql = "insert into retention_fee(`date`,si,gi,numup) values(?,?,?,?)";
        $res = $this->go($sql, 'i', [$date, $si, $gi, $numUp]);
        if ($res !== false) {
            txt_put_log('retention_fee', '插入数据成功', '记录时间：' . date('Y-m-d H:i:s') . ',新增服id：' . $si.',新增平台id：' . $gi);  //日志记录
            unset($res);
        } else {
            $errData[] = $si . '_' . $gi;
        }
    ...
    function updateRetention2($date, $n, $si, $gi)
    {
        ...
        $rColumn = 'r' . $n;
        $sql = "update retention_fee set $numInColumn=?, $rColumn=? where `date`=?  and `gi`=? and `si` = ?";

        $res = $this->go($sql, 'u', [$numIn, $r, $dateN, $gi, $si]);
        if ($res !== false) {
            txt_put_log('retention_fee', '更新成功', '记录时间：' . date('Y-m-d H:i:s') . ',修改内容：' . $numIn . '_' . $r.',更新服id：' .$si. ',更新平台id：' . $gi. ',留存第几天：' . $n);  //日志记录
        } else {
            $errData[] = $si . '_' . $gi;
        }
    ...
    {
        $sql1 = "SELECT `code` FROM `bill`  WHERE pay_time>=? AND pay_time < ? AND si in (".$si.")";
        $sql2 = '';
        $sql3 = '';
        $param = [
            strtotime($date),
