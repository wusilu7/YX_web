...
class DailyModel2 extends XoaModel
{
    ...
    function getDaily($time_start = '', $time_end = '')
    {
        ...
        foreach ($this->group_id as $gi){
            $url = "http://croodsadmin.xuanqu100.com/?p=I&c=Resource&a=selectDaily";
            if($gi==10){
                $url = "http://croodsadmin-lufeifan.xuanqu100.com/?p=I&c=Resource&a=selectDaily";
            }
            if($gi==54){
                $url = "http://croodsadmin-lehao.xuanqu100.com/?p=I&c=Resource&a=selectDaily";
            }
            if($gi==9||$gi==47||$gi==48||$gi==49||$gi==50||$gi==52||$gi==53||($gi>=55&&$gi<=64)){
                $url = "http://croodsadmin-juzhang.xuanqu100.com/?p=I&c=Resource&a=selectDaily";
            }
            if(($gi>=100&&$gi<=120)||$gi==46||$gi==44||$gi==45){
                $url = "http://croodsadmin-channel.xuanqu100.com/?p=I&c=Resource&a=selectDaily";
            }
            $param = [
                'gi'=>$gi,
