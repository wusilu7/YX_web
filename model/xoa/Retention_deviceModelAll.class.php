...
class Retention_deviceModelAll extends XoaModel
{
    ...
    function getRetention($time_start = '', $time_end = '')
    {
        ...
        foreach ($this->group_id as $gi){
            $url = "http://croodsadmin.xuanqu100.com/?p=I&c=Resource&a=retentionDeviceAll";
            if($gi==10){
                $url = "http://croodsadmin-lufeifan.xuanqu100.com/?p=I&c=Resource&a=retentionDeviceAll";
            }
            if($gi==54){
                $url = "http://croodsadmin-lehao.xuanqu100.com/?p=I&c=Resource&a=retentionDeviceAll";
            }
            if($gi==9||$gi==52||$gi==53||($gi>=55&&$gi<=61)){
                $url = "http://croodsadmin-juzhang.xuanqu100.com/?p=I&c=Resource&a=retentionDeviceAll";
            }
            if($gi>=100&&$gi<=120){
                $url = "http://croodsadmin-channel.xuanqu100.com/?p=I&c=Resource&a=retentionDeviceAll";
            }
            $param = [
                'gi'=>$gi,
