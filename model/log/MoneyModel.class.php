...
class MoneyModel extends LogModel
{
    /*局内数据查询*/
    function gameInnerData()
    {
        ...
        $opt = POST('opt');
        $DieType = POST('DieType');
        $stage_id = POST('stage_id');
        $sql1 = "select * from playeroutinfo ";
        $sql2 = " where log_time>= ? ";
...
//            var_dump($sql);die;
            ...
            //页数
            $sql1 = "select count(*) from playeroutinfo ";
            $sqlCount = $sql1 . $sql2;
            ...
            if ($count > 0) { 
                ...
                global $configA;
                foreach ($arr as $a=>$i) {
                    $arr[$a]['char_name'] = hex2bin($arr[$a]['char_name']);
                    if($arr[$a]['DieType'] == 0)
                    {
                        $arr[$a]['DieType'] = '死亡';
                    }elseif ($arr[$a]['DieType'] == 1)
                    {
                        $arr[$a]['DieType'] = '通关';
                    }elseif ($arr[$a]['DieType'] == 2)
                    {
                        $arr[$a]['DieType'] = '退出';
                    }elseif ($arr[$a]['DieType'] == 3)
                    {
                        $arr[$a]['DieType'] = '切换账号';
                    }
                }
    ...
    function selectMoney()
    {
        ...
        global $configA;
        $money = $configA[6];
        $money = array_column($money, null, 'val');
        unset($pi);
        unset($timeStart);
        unset($timeEnd);
        unset($char);
    ...
    function selectConsume()
    {
        ...
        $timeEnd    = POST('time_end') ? POST('time_end') : date("Y-m-d",strtotime("+1 day"));
        $money_type = POST('money_type');
        $check_type = POST('check_type') ? POST('check_type') : GET('jinIf');  // 查询类型
        if ($money_type+0 == 0)
        {
            $sql2 = " where opt=1  and `log_time`>='".$timeStart."' and `log_time`<'".$timeEnd."' ";
        }else
        {
            $sql2 = " where opt=1 and `currency_type`=".$money_type." and `log_time`>='".$timeStart."' and `log_time`<'".$timeEnd."' ";
        }
        // 查询单个平台的时候，过滤非该平台的角色id
        if ($pi > 0) {
        ...
        if($pi > 0){
            if ($money_type+0 == 0)
            {
                $sql="select currency_opt,count(distinct char_guid) char_num,count(real_amount) consume_num,sum(real_amount) amount from money where opt=1 and `log_time`>='".$timeStart."' and `log_time`<'".$timeEnd."' and `base_device_type`='".$pi."' group by currency_opt order by amount desc";
            }else
            {
            ...
            if ($money_type+0 == 0)
            {
                $sql="select currency_opt,count(distinct char_guid) char_num,count(real_amount) consume_num,sum(real_amount) amount from money where opt=1   and `log_time`>='".$timeStart."' and `log_time`<'".$timeEnd."' group by currency_opt order by amount desc";
            }else
            {
        ...
        if ($check_type == 912) {
            ...
            $dm2 = new Data2Model;
            $res = $dm2->logSummary($sql);
        }

        $sql_res = [];
        $currency_opt = array_unique(array_column($res,'currency_opt'));
        foreach ($res as $r) {
            foreach ($currency_opt as $co){
                if($r['currency_opt']==$co){
                    @$sql_res[$r['currency_opt']]['char_num'] += $r['char_num'];
                    @$sql_res[$r['currency_opt']]['consume_num'] += $r['consume_num'];
                    @$sql_res[$r['currency_opt']]['amount'] += $r['amount'];
                }
            }
        }
        global $configA;
        ...
        if ($check_type == 912) {
            ...
            $res = $dm2->data2Query($sql, 'amount', 'log');
        }
        $sum = implode($res);
        array_push($arr, $sum);
    ...
    function shopLog(){
        ...
        $sql1 = "SELECT SUM(a.real_amount) as sum,COUNT(DISTINCT a.char_guid) as char_num,COUNT(a.real_amount) as num,b.currency_type as buy_type FROM `money` AS a LEFT JOIN money AS b ON a.trans_id = b.trans_id";
        $sql2 = " WHERE  a.log_time >= ? AND a.log_time<? AND a.currency_type = 2 AND a.currency_opt = 58 AND b.log_time >= ? AND  a.log_time<? AND b.currency_type in (3,6,7,9,10) AND b.currency_opt = 2071";
        $sql3 = "  GROUP BY b.currency_type";
        $arr1 = $this->go($sql1.$sql2.$sql3,'sa',[$timeStart,$timeEnd,$timeStart,$timeEnd]);

        $sql1 = "SELECT SUM(a.real_amount) as amount  FROM `money` AS a LEFT JOIN money AS b ON a.trans_id = b.trans_id";
        $sum1 = implode($this->go($sql1 . $sql2, 's', [$timeStart,$timeEnd,$timeStart,$timeEnd]));

        $sql1 = "SELECT SUM(a.real_amount) as sum,COUNT(DISTINCT a.char_guid) as char_num,COUNT(a.real_amount) as num,b.new_item_id as buy_type FROM `money` AS a LEFT JOIN item AS b ON a.trans_id = b.trans_id";
        $sql2 = " WHERE   a.log_time >= ? AND a.log_time<? AND a.currency_type = 2 AND a.currency_opt IN (58,16) AND b.log_time >= ? AND  a.log_time<? AND b.log_source IN (1013, 1050)";
        $sql3 = " GROUP BY b.new_item_id";
        $arr2 = $this->go($sql1.$sql2.$sql3,'sa',[$timeStart,$timeEnd,$timeStart,$timeEnd]);

        $sql1 = "SELECT SUM(a.real_amount) as amount FROM `money` AS a LEFT JOIN item AS b ON a.trans_id = b.trans_id";
        $sum2 = implode($this->go($sql1 . $sql2, 's', [$timeStart,$timeEnd,$timeStart,$timeEnd]));
        ...
        foreach ($arr as &$r){
            if($r['buy_type']<=1000){
                @$r['buy_type'] = $configA[6][$r['buy_type']]['coin'];
            }else{
                if (array_key_exists($r['buy_type'], $item)) {
                    $r['buy_type'] = $item[$r['buy_type']][0].'('.$r['buy_type'].')';
                }
            }
        }
        array_push($arr, ($sum1+$sum2));
        return $arr;
    }
}
